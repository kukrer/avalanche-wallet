...
import { getConfigFromUrl, setNetworkAsync } from '@savannah-labs/savannah-wallet-sdk'
...
const network_module: Module<NetworkState, RootState> = {
    ...
    actions: {
        ...
        async init({ state, commit, dispatch }) {
            const savannah = new AvaNetwork(
                'Savannah',
                'http://api.savannah.network:9650',
                6,
                'http://api.savannah.network:9650',
                'http://explorer.savannah.network',
                true
            )

            const marula = new AvaNetwork(
                'Marula',
                'http://marularpc.savannah.network:9650',
                7,
                'http://marularpc.savannah.network:9650',
                'http://explorer.savannah.network',
                true
